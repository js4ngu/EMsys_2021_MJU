...
static unsigned long *pfbmap1; //프레임 버퍼
...
int fb_init(int *screen_width, int *screen_height, int *bits_per_pixel, int *line_length)
{
    ...
    if ((fbfd1 = open(FBDEV_FILE1, O_RDWR)) < 0)
    {
        printf("%s: open error\n", FBDEV_FILE1);
        return -1;
    }

    if (ioctl(fbfd1, FBIOGET_VSCREENINFO, &fbInfo))
    {
        printf("%s: ioctl error - FBIOGET_VSCREENINFO \n", FBDEV_FILE1);
        close(fbfd1);
        return -1;
    }
    if (ioctl(fbfd1, FBIOGET_FSCREENINFO, &fbFixInfo))
    {
    ...
    fbInfo.yoffset = 0;
    ioctl(fbfd1, FBIOPUT_VSCREENINFO, &fbInfo); //슉!
    ...
    *line_length = fbFixInfo.line_length;
    ...
    pfbmap1 = (unsigned long *)
        mmap(0, PFBSIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fbfd1, 0);

    if ((unsigned)pfbmap1 == (unsigned)-1)
    {
...
void fb_clear(void)
{
    ...
    {
        unsigned long *ptr = pfbmap1 + currentEmptyBufferPos + (fbWidth * coor_y);
...
void fb_clear2(int xpos, int ypos, int width, int height)
{
    ...
    //sheight
    if (ypos + height > fbHeight)
        height = fbHeight - ypos;
...
void fb_write(char *picData, int picWidth, int picHeight)
{
    ...
    for (coor_y = 0; coor_y < targetHeight; coor_y++)
    {
        ...
        for (coor_x = 0; coor_x < targetWidth; coor_x++)
        {
            ...
            pfbmap1[coor_y * fbWidth + (fbWidth - coor_x - 1)] =
